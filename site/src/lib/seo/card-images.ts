 ...
 */
function relevance(cardTokens: Set<string>, template: SerializedTemplate): number {
...
function firstStill(template: SerializedTemplate): string | null {
  return template.thumbnails?.find((thumb) => !isMediaFile(thumb)) ?? null;
}

function toCardImage(template: SerializedTemplate): CardImage | null {
  const still = firstStill(template);
  return still ? { src: thumbnailPath(still), alt: template.title || template.name } : null;
}
...
export interface AssignOptions {
   ...
   */
  fallback?: SerializedTemplate[];
}

const withStill = (templates: SerializedTemplate[]) =>
  templates
...
/** Convenience: just the matched image per card (see `assignCardTemplates`). */
export function assignCardImages(
  cards: CardText[],
  templates: SerializedTemplate[],
  opts: AssignOptions = {}
): (CardImage | null)[] {
  return assignCardTemplates(cards, templates, opts).map((t) => (t ? toCardImage(t) : null));
}
