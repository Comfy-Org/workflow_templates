...
import { deriveModelGroups } from './src/lib/seo/model-groups.ts';

const templatesDir = path.join(process.cwd(), 'src/content/templates');
const modelContentDir = path.join(process.cwd(), 'src/content/seo/models');
  ...
  integrations: [
    sitemap({
      ...
      serialize(item) {
        ...
        if (pathname.match(/^\/workflows\/use-case\//)) {
          // @ts-expect-error - sitemap types are stricter than actual API
          item.changefreq = 'weekly';
          item.priority = 0.8;
          return item;
