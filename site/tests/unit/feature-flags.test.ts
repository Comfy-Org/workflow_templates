...
import type { FeatureFlagsSnapshot } from '../../src/data/feature-flags';
...
import bundledSnapshot from '../../src/data/feature-flags.snapshot.json' with { type: 'json' };
...
describe('featureFlags (synchronous consumer)', () => {
  it('mirrors cloudFreeTier from the build-time snapshot', () => {
    expect(featureFlags.cloudFreeTier).toBe(bundledSnapshot.flags.cloudFreeTier);
  });
...
describe('fetchFeatureFlagsForBuild', () => {
  ...
  it('defaults cloudFreeTier to false when the flag is absent from /features', async () => {
    const fetchImpl = vi.fn(async () => response({ partner_node_conversion_rate: 0.05 }));
    const outcome = await fetchFeatureFlagsForBuild({
      baseUrl: BASE_URL,
      fetchImpl: fetchImpl as unknown as typeof fetch,
    });
