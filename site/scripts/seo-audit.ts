...
function printReport(reports: PageReport[]): void {
  ...
  for (const report of reports) {
    ...
    pagesWithIssues++;
    console.log(`${colors.bold}${colors.magenta}📄 ${report.file}${colors.reset}`);
    console.log(`${colors.gray}${'─'.repeat(60)}${colors.reset}`);

    for (const issue of report.issues) {
      const color = severityColor(issue.severity);
      console.log(`   ${severityIcon(issue.severity)} ${color}${issue.message}${colors.reset}`);
  ...
  console.log(`   ${colors.gray}Pages scanned:${colors.reset}     ${reports.length}`);
  console.log(`   ${colors.gray}Pages with issues:${colors.reset} ${pagesWithIssues}`);
  console.log();
  console.log(`   ${colors.red}✖ Critical:${colors.reset}        ${totalCritical}`);
  console.log(`   ${colors.yellow}⚠ Warnings:${colors.reset}        ${totalWarning}`);
...
function main(): void {
  ...
  if (htmlFiles.length === 0) {
    console.error(`${colors.red}Error: No HTML files found in dist/${colors.reset}`);
    process.exit(1);
  }

  console.log(`${colors.gray}Found ${htmlFiles.length} HTML file(s)${colors.reset}`);
