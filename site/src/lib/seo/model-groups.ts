...
import type { KeywordModel, SeoPageCard } from './schema';
...
const FAMILY_RULES: { match: RegExp; label: string }[] = [
  ...
  { match: /^sd(xl|3|\b)|^stable-diffusion/i, label: 'Stable Diffusion' },
...
/**
 * Qualifying model families as index cards, usage-sorted. Mirrors
 * `resolveUseCasePageCards` (same `SeoPageCard` shape), so the model index reuses
 * the use-case card markup. Only qualifying families appear, so every card links
 * a rich, indexable page rather than a bare noindex grid.
 */
