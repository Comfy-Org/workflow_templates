...
async function fallback(reason: string, snapshotUrl: URL | undefined): Promise<FetchOutcome> {
...
async function tryFetchAndParse(options: FetchFeatureFlagsOptions): Promise<FetchOk | FetchErr> {
...
async function callOnce(
  ...
  try {
    ...
    const reason = error instanceof Error ? `network error: ${error.message}` : 'network error';
...
async function readSnapshot(snapshotUrl: URL | undefined): Promise<FeatureFlagsSnapshot | null> {
  if (snapshotUrl) {
    try {
