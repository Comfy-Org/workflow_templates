...
function buildAnnotations(outcome: FetchOutcome): string[] {
  ...
  return [
    `::error title=Feature flags fetch failed and no snapshot is available::Cannot build site without feature flags.%0A%0AReason: ${escapeAnnotation(outcome.reason)}%0A%0AAction items:%0A  1. Run \`pnpm feature-flags:refresh-snapshot\` locally.%0A  2. Commit site/src/data/feature-flags.snapshot.json.%0A  3. Push and re-run CI.`,
  ];
}
...
function staleAnnotation(reason: string): string {
  ...
  if (reason.startsWith('schema')) {
    return `::error title=Feature flags schema mismatch::${escaped}. The /features API contract has likely changed. Build continues with the snapshot, but future updates will fail until the schema is fixed.%0A%0AAction items:%0A  1. Inspect the response at https://api.comfy.org/features.%0A  2. Update site/src/utils/featureFlags.schema.ts to match the new shape.`;
  }
