 ...
 */
export function getVariantSync(experimentId: string, fallbackConfig: Experiment): string {
  // 1. Check URL param override
  const urlVariant = getVariantFromUrl(experimentId);
  if (urlVariant) {
    return urlVariant;
 ...
 */
export function createExperimentHandler(experimentId: string, fallbackConfig: Experiment) {
  return {
