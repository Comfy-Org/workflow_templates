...
function validateTemplate(
  ...
  if (typeof template.name !== 'string' || !template.name) {
    errors.push({ path: prefix, message: 'missing or invalid "name" (required string)' });
  }
  ...
  if (
    !template.mediaType ||
    !(VALID_MEDIA_TYPES as readonly string[]).includes(template.mediaType as string)
  ) {
    errors.push({
      path: `${prefix}(${template.name})`,
  ...
  if (template.thumbnailVariant !== undefined) {
    if (
      !(VALID_THUMBNAIL_VARIANTS as readonly string[]).includes(template.thumbnailVariant as string)
    ) {
      errors.push({
        path: `${prefix}(${template.name})`,
  ...
  if (template.date !== undefined) {
    if (typeof template.date !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(template.date)) {
      errors.push({
        path: `${prefix}(${template.name})`,
  ...
  if (template.openSource !== undefined && typeof template.openSource !== 'boolean') {
    errors.push({ path: `${prefix}(${template.name})`, message: '"openSource" must be a boolean' });
  }

  if (template.logos !== undefined) {
    if (!Array.isArray(template.logos)) {
      errors.push({ path: `${prefix}(${template.name})`, message: '"logos" must be an array' });
    } else {
      for (const logo of template.logos) {
        if (typeof logo !== 'object' || logo === null || !('provider' in logo)) {
          errors.push({
            path: `${prefix}(${template.name})`,
  ...
  if (template.requiresCustomNodes !== undefined) {
    ...
    } else if (!template.requiresCustomNodes.every((n: unknown) => typeof n === 'string')) {
      errors.push({
        path: `${prefix}(${template.name})`,
...
function validateModule(mod: ModuleEntry, index: number): ValidationError[] {
  const errors: ValidationError[] = [];
  const moduleName = typeof mod.moduleName === 'string' ? mod.moduleName : `index:${index}`;

  if (typeof mod.moduleName !== 'string' || !mod.moduleName) {
    errors.push({ path: `modules[${index}]`, message: 'missing or invalid "moduleName"' });
  }
  ...
  for (let i = 0; i < mod.templates.length; i++) {
    errors.push(...validateTemplate(mod.templates[i] as TemplateEntry, i, moduleName));
  }
...
function main() {
  const indexPath = resolve(import.meta.dirname!, '../../templates/index.json');
  let raw: string;
  try {
  ...
  for (const name of allNames) {
    if (seen.has(name)) {
      errors.push({ path: 'global', message: `duplicate template name: "${name}"` });
    }
    seen.add(name);
  }
