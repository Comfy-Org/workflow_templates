/**
 * Right-size the hub's card images. Companion to hub-media-build.mjs.
 *
...
const gridPath = arg('grid'), outDir = arg('out'), manifestPath = arg('manifest');
...
for (const t of grid) for (const u of t.thumbnails ?? [])
  if (!isVideo(u) && isImage(u)) sources.set(idOf(u), u);

const manifest = fs.existsSync(manifestPath)
  ? JSON.parse(fs.readFileSync(manifestPath, 'utf8')) : {};
console.log(`${sources.size} image assets, ${Object.keys(manifest).length} already done`);

let done = 0, skipped = 0, animated = 0, transparent = 0, failed = 0, bIn = 0, bOut = 0;
...
for (const [id, url] of sources) {
  ...
  try {
    ...
    const probe = JSON.parse((await run(FFPROBE, ['-v', 'error', '-select_streams', 'v:0',
      '-show_entries', 'stream=codec_name,width,height,pix_fmt', '-of', 'json', src])).stdout);
    ...
    if (/_anim$/.test(codec) || codec === 'gif') {
      ...
      animated++; console.log(`  ${id.slice(0, 8)}  skip: animated ${w}x${h}`); continue;
    }

    // An rgba PNG is usually fully opaque; only real transparency blocks JPEG.
    if (/a$|rgba|argb/.test(pix ?? '')) {
      const out = await run(FFMPEG, ['-hide_banner', '-i', src, '-vf',
      ...
      if (min !== undefined && Number(min) < 255) {
        ...
        transparent++; console.log(`  ${id.slice(0, 8)}  skip: real transparency`); continue;
      }
    }
    ...
    await run(FFMPEG, ['-hide_banner', '-loglevel', 'error', '-y', '-i', src,
      '-vf', vf, '-q:v', String(JPEG_Q), '-pix_fmt', 'yuvj420p', outFile]);
    ...
    if (outBytes >= srcBytes) {
      ...
      skipped++; console.log(`  ${id.slice(0, 8)}  skip: no gain`); continue;
    }

    manifest[id] = { image: `${PUBLIC_BASE}/images/${id}.jpg`, w: Math.min(w, MAX_WIDTH), srcBytes, outBytes };
    bIn += srcBytes; bOut += outBytes; done++;
    console.log(`  ${id.slice(0, 8)}  ${w}x${h} ${(srcBytes / 1048576).toFixed(2)}MB -> ${(outBytes / 1024).toFixed(0)}KB  (${(100 - outBytes * 100 / srcBytes).toFixed(1)}% off)`);
  } catch (err) {
    failed++; manifest[id] = { skip: 'error', note: String(err.message).slice(0, 100) };
